...
def register_model(model_path, encoders_path, metrics_path, feature_cols_path, model_name):
    ...
    with mlflow.start_run(run_name=f"{model_name}_registration") as run:
        ...
        # Guardar el modelo como .pth y subirlo a S3
...
# Registers the trained model in MLflow with metrics and metadata directly from S3.
...
# from model import EstablishmentDNN  # Ajusta tu path si es necesario
...
#     """Download a file from S3 to a local path (Windows compatible)."""
#     s3 = boto3.client("s3")
...
#     s3.download_file(bucket, key, local_path)
#     logger.success(f"Downloaded {s3_uri} to {local_path}")
#     return local_path
...
#     # -------------------------------
#     tmp_dir = Path(os.environ.get("TMP", "C:/Users/CAMILO/AppData/Local/Temp"))
#     local_model = str(tmp_dir / "model.pth")
#     local_encoders = str(tmp_dir / "encoders.pkl")
#     local_metrics = str(tmp_dir / "metrics.json")
#     local_features = str(tmp_dir / "feature_columns.pkl")

#     download_from_s3(model_path, local_model)
#     download_from_s3(encoders_path, local_encoders)
#     download_from_s3(metrics_path, local_metrics)
#     download_from_s3(feature_cols_path, local_features)
...
#     model = EstablishmentDNN(input_dim=input_dim, num_establishments=num_establishments)
#     model.load_state_dict(checkpoint['model_state_dict'])
#     model.eval()
...
#         # Log artifacts
#         mlflow.log_artifact(local_encoders, "encoders")
#         mlflow.log_artifact(local_metrics, "metrics")
#         mlflow.log_artifact(local_features, "feature_columns")
